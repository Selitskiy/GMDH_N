function [X2, Y2, Yh2, B, k_tob] = w_seriesva_test_tensors(M, m_in, n_out, l_sess, l_test, n_sess, sess_off, offset, norm_fl, m_ine, n_oute, k_tob)
    %% Test regression ANN
    if(k_tob == 0)
        k_tob = ceil(l_sess/n_out);
    end

    X2 = ones([m_ine, k_tob, n_sess-sess_off]);
